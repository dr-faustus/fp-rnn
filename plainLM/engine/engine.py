...
from construct import get_param_groups
...
class TorchEngine(torch.nn.Module):
  ...
  def __init__(
    ...
    self.ctx = nullcontext() if device_type == 'cpu' else torch.amp.autocast(device_type=device_type, dtype=ptdtype, cache_enabled=False)
  ...
  @torch.no_grad()
  def eval(self, dataloader, max_iter=None):
