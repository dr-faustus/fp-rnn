...
def reduce_metrics(metrics: list[dict[str, any]]) -> dict[str, any]:
    ...
    Args:
        metrics (list[dict[str, any]]): A list of metrics for each batch. The list should
    ...
    for k in metric_names:
        if 'fp_iters_layer' in k:
            weighted_metrics[k] = {
                "mean": weighted_metrics[k],
                "median": np.median([v[k]["value"] for v in metrics])
            }
...
def compute_metrics(
    ...
    if optimizer is not None:
        values_dict[prefix_str + 'lr'] = {
            'value': optimizer.param_groups[0].get("lr", float("NaN")), 
            'n_samples':1 
        }

    if isinstance(model, FPLMHeadModel):
        for idx, layer in enumerate(model.backbone.layers):
            # mean and median will be reduced in reduce_metrics()
            values_dict[prefix_str + 'fp_iters_layer' + str(idx)] = {
                'value': layer.mixer.fixed_point_iter[-1],
                'n_samples': 1
            }
            # values_dict[prefix_str + 'grad_norm_layer' + str(idx)] = {
            #     'value': torch.nn.utils.parameters_to_vector([param.grad.data for param in layer.parameters()]).norm(),
            #     'n_samples': 1
            # }

    if reduce_batch:
        values_dict = reduce_metrics([values_dict])
