...
class Trainer:
  def __init__(self, model, params, train_generator, eval_generator,
    ...
    self.best_eval_state_path = os.path.join(self.p.log_folder,
                                             BEST_MODEL_FILE_NAME + '-{}-{}-{}-{}-{}-{}-{}'.format(self.p.model_name, self.p.mixer_type, self.p.mixer_rank, self.p.d_model, self.p.n_layers, self.p.lr, self.p.seed))
    self.last_eval_state_path = os.path.join(self.p.log_folder,
                                             LAST_MODEL_FILE_NAME + '-{}-{}-{}-{}-{}-{}-{}'.format(self.p.model_name, self.p.mixer_type, self.p.mixer_rank, self.p.d_model, self.p.n_layers, self.p.lr, self.p.seed))
  ...
  def _forward(self, x, y, task_id, seq_len, voi):
    ...
    def compute_loss(logits, mask):
      ...
      if mask_flat.sum().item() == 0:
        ...
        loss = self.criterion(logits_flat[mask_flat], y.reshape(-1)[mask_flat])
      return loss
    ...
    _, ra_n_per_task, ra_correct_per_task = compute_accs(logits.cpu(), y.cpu(), task_id.cpu(), x.cpu() == self.p.QM)
    _, lm_n_per_task, lm_correct_per_task = compute_accs(logits.cpu(), y.cpu(), task_id.cpu(), x.cpu() != self.p.PAD)
  ...
  def train(self):
    ...
    while self.state.global_step < self.p.max_steps or self.p.max_steps == -1:
      ...
      # run forward
      loss = self._forward(x, y, task_id, seq_len, train_voi)
      ...
      if self.state.global_step % self.p.log_every_n_steps == 0:
        ...
        if self.p.use_wandb:
            metrics = {
                ...
                'train/avg_mem_norm': avg_mem_norm,
                'train/avg_mem_abs_max': avg_mem_abs_max,
                'train/avg_ra_loss': avg_ra_loss,
                'train/avg_lm_loss': avg_lm_loss,
                'train/avg_regularizer': avg_regularizer,
                'train/avg_ra_acc': avg_ra_acc,
                'train/avg_lm_acc': avg_lm_acc,
            ...
            # ra_accs: [21]
            try:
                for idx, acc in enumerate(ra_accs):
                    metrics['train/ra-acc-task_id_' + str(idx)] = acc
            except:
                metrics['train/ra-acc'] = ra_accs.item()
            ...
            # ra_accs: [21]
            try:
                for idx, acc in enumerate(lm_accs):
                    metrics['train/lm-acc-task_id_' + str(idx)] = acc
            except:
                metrics['train/lm-acc'] = lm_accs.item()
            if self.p.use_wandb:
      ...
      if self.state.global_step % self.p.eval_every_n_steps == 0:
        ...
        self.evaluate(use_wandb=self.p.use_wandb, eval_type='val')
        self.model.train()
  ...
  def evaluate(self, generator=None, use_wandb=False, progress=False, eval_type='val'):
    ...
    with torch.no_grad():
      ...
      for x, y, task_id, seq_len in generator:      
        ...
        _ = self._forward(x, y, task_id, seq_len, eval_voi)
    ...
    if use_wandb:
        prefix = eval_type + '/'
        metrics = {
            prefix + 'avg_loss': avg_loss,
            prefix + 'avg_mem_norm': avg_mem_norm,
            prefix + 'avg_mem_abs_max': avg_mem_abs_max,
            prefix + 'avg_ra_loss': avg_ra_loss,
            prefix + 'avg_lm_loss': avg_lm_loss,
            prefix + 'avg_regularizer': avg_regularizer,
            prefix + 'avg_ra_acc': avg_ra_acc,
            prefix + 'avg_lm_acc': avg_lm_acc,
            prefix + 'batches_per_sec': batches_per_sec,
            prefix + 'tokens_per_sec': tokens_per_sec
        }
        ...
        try:
            for idx, acc in enumerate(ra_accs):
                metrics[prefix + 'ra-acc-task_id_' + str(idx)] = acc
        except:
            metrics[prefix + 'ra-acc'] = ra_accs.item()
        ...
        try:
            for idx, acc in enumerate(lm_accs):
                metrics[prefix + 'lm-acc-task_id_' + str(idx)] = acc
        except:
            metrics[prefix + 'lm-acc'] = lm_accs.item()
        wandb.log(metrics, step=self.state.global_step)
