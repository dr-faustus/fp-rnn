    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in progress_bar:
            ...
            if step % gradient_accumulation_steps == 0:
                ...
                optimizer.zero_grad()
                completed_steps += 1
            if args.use_wandb and step % args.update_wandb_interval == 0:
                ...
                wandb.log(metrics, step=epoch * len(train_dataset) + step)
            if step > num_training_steps:
                break
            # Update tqdm description with the current loss
            if 'FP' in args.model:
                progress_bar.set_postfix({'Loss': loss.item(), 'FP itrs': int(np.median(model.backbone.layers[0].mixer.fixed_point_iter))})
            else:
...
def save_model(args, model):
    ...
    if args.model.startswith("T"):
        ...
        save_path = "./output_dir/"+f"model_{args.model}_layer_{args.layers}_hidden_{args.hidden_size}_train_{args.train_task}_lr_{args.lr}_epochs_{args.epochs}_steps_{args.steps}_mixerrank_{args.mixer_rank}/"
