...
Source: https://github.com/jopetty/word-problem/blob/8f910f92e1c70455dcd9376f56032dfc55126188/src/generate_data.py
"""
...
def main(
    ...
    data_dir: str | Path = Path(os.environ["DATA"]) / "word_problem",
    ...
    if samples is None:
        ...
        sequences = set()
        for _ in tqdm(range(samples), desc="Sampling Sequences"):
            sequences.add(tuple(random.choices(range(num_elements), k=k)))
        sequences = list(sequences)

    examples = []
    for seq in tqdm(sequences, desc="Generating Targets"):
        acc = 0
